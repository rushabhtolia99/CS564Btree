...
void test2();
void test3();
void largeTreeConstructTest(); // Selfmade test #1
void searchKeyTest(); //Selfmade test #2
...
void searchKeyTest() {
	std::cout << "--------------------" << std::endl;
	std::cout << "catchRelationForwardValue" << std::endl;
	catchRelationForwardValue(5000);
	indexTests3();
	deleteRelation();
...
void intTests2()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,60000,GTE,70000,LT), 10000)
	checkPassFail(intScan(&index,0,GT,99999,LTE), 99999)
